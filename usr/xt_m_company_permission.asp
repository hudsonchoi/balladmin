...
Dim myMlb, myMinor, myAlls, myCoop, myWorld, myMascot, mySpring, myTrend, mySuper, myMlb2003, myCreative, myAlls2007
...
For i=1 to Request.Form("section").Count 'Set the permission to be open
	...
	If Request.Form("section")(i) = "Creative" then
		myCreative = 1
	end if
	If Request.Form("section")(i) = "Alls2007" then
		myAlls2007 = 1
	end if
next
...
if not isEmpty (Request.Form("goback")) then 'Go back
	...
	"&23=" & Server.URLEncode(myMlb2003) & _
	"&24=" & Server.URLEncode(myCreative) & _
	"&25=" & Server.URLEncode(myAlls2007)
...
if Request.Form("modify") then
	...
	", licensee.Creative = " & myCreative & _
	", licensee.Alls2007 = " & myAlls2007 & _
...
If Err.number <> 0 Then
	...
	strBody = strBody & "Creative:" & myCreative
